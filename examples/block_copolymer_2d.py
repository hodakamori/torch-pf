...
def main() -> None:
    ...
    print("Running lamellae (φ̄ = 0.5) ...")
    solver_lam, snaps_lam = run_ok(0.5, params, free_energy, alpha, device, n_steps, save_interval)

    # --- Cylinders / dots (asymmetric, φ̄ = 0.35) ---
    print("Running cylinders (φ̄ = 0.35) ...")
    solver_cyl, snaps_cyl = run_ok(0.35, params, free_energy, alpha, device, n_steps, save_interval)
    ...
    for row, (solver, snaps, label) in enumerate([
        (solver_lam, snaps_lam, r"Lamellae ($\bar\phi=0.5$)"),
        (solver_cyl, snaps_cyl, r"Cylinders ($\bar\phi=0.35$)"),
    ]):
    ...
    fig.suptitle(
        rf"Ohta-Kawasaki Model ($\alpha={alpha}$, $\kappa={params.kappa}$, $W={free_energy.W}$)",
        fontsize=14,
    )
